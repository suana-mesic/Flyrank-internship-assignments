...
builder.Services.AddSingleton(new ReportRepository(connStr));
...
// Temporary: returns the report data as JSON so we can verify the aggregation.
app.MapGet("/reports/preview", (HttpContext http, string period,
    AuthService auth, ReportRepository reports, PricingService pricing) =>
{
    var token = ExtractToken(http);
    if (token is null) return Results.Json(new { error = "Access token required" }, statusCode: 401);
    var tenantId = GetTenantId(auth.ValidateToken(token));
    if (tenantId is null) return Results.Json(new { error = "Invalid token" }, statusCode: 401);

    var data = reports.GetReportData(tenantId.Value, period);
    if (data is null) return Results.NotFound(new { error = "No data for tenant/period" });

    // Simple cost: calls + tokens (tokens at output rate), like the early /usage rollup.
    var cost = pricing.CalculateCost(data.ApiCallsUsed, 0, 0, data.TokensUsed);
    return Results.Ok(data with { Cost = cost });
});

// Temporary: generate and return the PDF inline so we can eyeball it.
app.MapGet("/reports/pdf", (HttpContext http, string period,
    AuthService auth, ReportRepository reports, PricingService pricing, ReportService report) =>
{
    var token = ExtractToken(http);
    if (token is null) return Results.Json(new { error = "Access token required" }, statusCode: 401);
    var tenantId = GetTenantId(auth.ValidateToken(token));
    ...
    var cost = pricing.CalculateCost(data.ApiCallsUsed, 0, 0, data.TokensUsed);
    var pdf = report.GeneratePdf(data with { Cost = cost });

    return Results.File(pdf, "application/pdf", $"report-{data.TenantEmail}-{period}.pdf");
});

// Enqueue a report job; returns immediately (202) with the job id.
app.MapPost("/reports", (HttpContext http, string period, AuthService auth, ReportRepository reports) =>
{
    var token = ExtractToken(http);
    if (token is null) return Results.Json(new { error = "Access token required" }, statusCode: 401);
    var tenantId = GetTenantId(auth.ValidateToken(token));
    ...
    var id = reports.CreatePending(tenantId.Value, period);
    return Results.Accepted($"/reports/{id}", new { id, status = "pending" });
});

// Job status + download link when ready.
app.MapGet("/reports/{id:int}", (int id, HttpContext http, AuthService auth, ReportRepository reports) =>
{
    var token = ExtractToken(http);
    if (token is null) return Results.Json(new { error = "Access token required" }, statusCode: 401);
    var tenantId = GetTenantId(auth.ValidateToken(token));
    ...
    var r = reports.GetById(id);
    if (r is null || r.Value.tenantId != tenantId.Value) return Results.NotFound();
    return Results.Ok(new
    {
        r.Value.id,
        r.Value.status,
        downloadUrl = r.Value.status == "done" ? $"/reports/{r.Value.id}/download" : null,
        r.Value.error
    });
});

// Stream the stored PDF from disk.
app.MapGet("/reports/{id:int}/download", (int id, HttpContext http, AuthService auth, ReportRepository reports) =>
{
    var token = ExtractToken(http);
    if (token is null) return Results.Json(new { error = "Access token required" }, statusCode: 401);
    var tenantId = GetTenantId(auth.ValidateToken(token));
    ...
    var r = reports.GetById(id);
    if (r is null || r.Value.tenantId != tenantId.Value) return Results.NotFound();
    if (r.Value.status != "done" || r.Value.filePath is null || !System.IO.File.Exists(r.Value.filePath))
        return Results.Json(new { error = "Report not ready" }, statusCode: 409);

    return Results.File(r.Value.filePath, "application/pdf", $"report-{r.Value.period}.pdf");
});
